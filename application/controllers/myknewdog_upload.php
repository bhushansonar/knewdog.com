...
class Myknewdog extends CI_Controller {
    ...
    */
    ...
    public function __construct()
    {
        ...
        if(!$this->session->userdata('is_logged_in')){
			$this->session->set_flashdata('flash_message', ''._clang(PLEASE_LOGIN_TO_GET_IN).'');
    ...
    */
    public function index()
    {
		...
		$mynl_config["uri_segment"] = 2;
        $mynl_config['base_url'] = base_url().'mynewdog';
		...
		if($this->uri->segment(2) || $this->input->post('form') == 'section_2'){
			$data['mynl_tab'] = 'tab_2';
			//$this->session->set_flashdata('flash_mynl_tab', 'tab_2');
			}else if($this->input->post('form') == 'section_3'){
				$data['mynl_tab'] = 'tab_3';
			}else if($this->input->post('form') == 'section_4'){
				$data['mynl_tab'] = 'tab_4';
				}
	...
	//	echo "<br>";
		...
		if($mynl_search_string !== false && $mynl_order !== false || $this->uri->segment(2) == true){ 
          ...
          $mynl_config['total_rows'] = $data['mynl_count_products'];
            ...
            @$data['mynl_newsletter'] = $this->newsletter_model->get_newsletter_front('', '', $mynl_order_type, $mynl_config['per_page'],$mynl_limit_end,'Active',$mynl_where_field,$mynl_where_value,$mynl_language_id,$mynl_newsletter_category_id,$mynl_rating_id,$mynl_author_country,$mynl_author_zipcode,$mynl_where_in_field,$mynl_where_in_value);        
		...
		//if($this->input->post('schedule_id')){
			...
			//}else{
				//$data['edit_schedule'] = '';
				//}
		$data['main_content'] = 'myknewdog_view';
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
           ...
           // {
				...
				foreach($array as $k=>$v){
					//echo $v; die;
 					 if (in_array($v,array('Submit'))) 
					 	{unset($array[$v]);}
					if(is_array($v)){
						$array[$k] = implode(",",$v);	
						}
					 if (in_array($k,array('language_id'))) 
					 	{
							
							$user_d = $this->user_model->get_user_by_id($id);
						if(!empty($user_d[0][$k])){
							$dat = explode(",",$user_d[0][$k]);
							array_push($dat,$array[$k]);
							$language_ids = implode(",",$dat);
							//echo '<pre>';print_r($dat); 
							
						}else{
							$language_ids = $array[$k];
							}
							unset($array['language_id']);
							$array['language_id'] = $language_ids;
							//echo '<pre>'; print_r($array['language_id']);die;
						
						}
					 if (in_array($k,array('user_interests'))) 
					 	{
							$user_d = $this->user_model->get_user_by_id($id);
							if(!empty($user_d[0][$k])){
								$dat = explode(",",$user_d[0][$k]);
							...
							$array['user_interests'] = $language_ids;
  ...
  //echo "hi"; die;
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
           ...
           if ($this->form_validation->run())
			...
			if($mode == 'Update'){	
			    if($this->additional_email_model->update_additional_email($additional_email_id, $array) == TRUE && $type_of_membership != 'FREE' ){
                    $this->session->set_flashdata('flash_message', ''._clang(MY_PROFILE_UPDATED_WITH).'');
					$this->session->set_flashdata('flash_class', 'alert-success');
					$this->session->set_flashdata('flash_mynl_tab', 'tab_1');
                }else{
				...
				if(@$this->additional_email_model->store_additional_email($array) == TRUE && $type_of_membership != 'FREE'){
  ...
  //echo "hi"; die;
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
           ...
           if ($this->form_validation->run())
           {
				...
				//print_r($array); die;
			    if(@$this->user_model->update_user($id, $array) == TRUE){
					$get_lang_a = $this->user_model->get_user_by_id($id);
	...
	function schedule_edit(){
		 ...
		 if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
            ...
            if ($this->form_validation->run())
            {
				...
				$weeks_on = implode(",",$this->input->post('weeks_on'));
				$ends = $this->input->post('ends');
					if($ends == 'Never'){
						$end_after = '';
						$ends_on = '';
					}elseif($ends == "after"){
						$end_after = $this->input->post('ends_after');
...
//print_r($_FILES); die;
    ...
    if ($status != "error")
    {
		...
		$user_avatar= $this->user_model->get_user_by_id($user_id);
        if (isset($data['error'])) 
		{
            $status = 'error';
            $msg = $this->upload->display_errors('', '');
			$return = array("status" => $status,"url" => $user_avatar[0]['avatar']);
        }
        else
        {
           // $data = $this->upload->data();
            ...
            if(!empty($file_id))
            {
				...
				$return = $return = array("status" => $status,"url" => $data['upload_data']['file_name']);
            }
            else
            {
               $status = "error";
        ...
        if ($_FILES) {
            ...
            if (! $_FILES['image_file']['error'] ) {
				...
				if(isset($data['error']))
				{
				   ...
				   //echo "file->".$imagename." ";
					$sTempFileName = $data['upload_data']['full_path'];//.$data['upload_data']['raw_name'];
