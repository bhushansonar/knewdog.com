...
//        exit;
        ...
        for ($r = 0; $r < count($rarray); $r++) {
            ...
            $email = !empty($rarray[$r]['to'][1]) ? decode_imap_text($rarray[$r]['to'][1]) : decode_imap_text($rarray[$r]['to'][0]['mailbox']) . "@" . decode_imap_text($rarray[$r]['to'][0]['host']); //$rarray[$r]['to'][0]->['mailbox']."@".$rarray[$r]['to'][0]->['host'];
...
//                    die;
                ...
                //pass_blacklist_index -> set
                $store_pass_data = array(
                    "newsletter_id" => $last_newsletter_id,
                    "flag" => 1,
                    "timestamp" => date("Y-m-d h:i:s")
                );
                $this->administration_folder_model->store_pass_blacklist_index($store_pass_data);
