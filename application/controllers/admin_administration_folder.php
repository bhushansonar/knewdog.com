...
class Admin_administration_folder extends CI_Controller {
    /**
    ...
    const VIEW_FOLDER = 'kd2a2a0u1g4/administration_folder';
    ...
    public function index() {
        ...
        $config['base_url'] = base_url() . 'kd2a2a0u1g4/administration_folder';
        ...
        if ($search_string !== false && $order !== false || $this->uri->segment(3) == true) {
            ...
            $data['count_products'] = $this->administration_folder_model->count_administration_folder($search_string, $order, $where_field, $where_value);
            $config['total_rows'] = $data['count_products'];

            //fetch sql data into arrays
            if ($search_string) {
                if ($order) {
                    $data['administration_folder'] = $this->administration_folder_model->get_administration_folder($search_string, $order, $order_type, $config['per_page'], $limit_end, '', $where_field, $where_value);
                } else {
                    $data['administration_folder'] = $this->administration_folder_model->get_administration_folder($search_string, '', $order_type, $config['per_page'], $limit_end, '', $where_field, $where_value);
                }
            } else {
                if ($order) {
                    $data['administration_folder'] = $this->administration_folder_model->get_administration_folder('', $order, $order_type, $config['per_page'], $limit_end, '', $where_field, $where_value);
                } else {
                    $data['administration_folder'] = $this->administration_folder_model->get_administration_folder('', '', $order_type, $config['per_page'], $limit_end, '', $where_field, $where_value);
                }
            }
        } else {

            //clean filter data inside section
            $filter_session_data['administration_folder_selected'] = null;
            ...
            //fetch sql data into arrays
            $data['count_administration_folder'] = $this->administration_folder_model->count_administration_folder('', '', $where_field, $where_value);
            $data['administration_folder'] = $this->administration_folder_model->get_administration_folder('', '', $order_type, $config['per_page'], $limit_end, '', $where_field, $where_value);
            $config['total_rows'] = $data['count_administration_folder'];
        ...
        //echo '<pre>';print_r($data['administration_folder']); die;

        $data['main_content'] = 'kd2a2a0u1g4/administration_folder/list';
...
//index
    ...
    public function issues() {
        ...
        $config['base_url'] = base_url() . 'kd2a2a0u1g4/administration_folder/issues';
        $config['use_page_numbers'] = TRUE;
        $config['num_links'] = 20;
        $config['full_tag_open'] = '<ul>';
        $config['full_tag_close'] = '</ul>';
        ...
        $get_newsletter_rand_id = $this->administration_folder_model->get_administration_folder_by_field('newsletter_id', $id);
        ...
        $data['main_content'] = 'kd2a2a0u1g4/administration_folder/issues';
        $this->load->view('kd2a2a0u1g4/includes/template', $data);
    }
...
//index
    ...
    public function add() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            //form validation
            $this->form_validation->set_rules('administration_folder_name', 'Newsletter name', 'required');
            ...
            if ($this->form_validation->run()) {
                ...
                $sn_id = $this->functions->get_administration_folder_lsn_id();
                $data_to_store = array(
                    ...
                    'administration_folder_name' => $this->input->post('administration_folder_name'),
                ...
                if ($this->administration_folder_model->store_administration_folder($data_to_store)) {
                    ...
                    redirect('kd2a2a0u1g4/administration_folder/');
        ...
        $data['main_content'] = 'kd2a2a0u1g4/administration_folder/add';
        $this->load->view('kd2a2a0u1g4/includes/template', $data);
    }
    ...
    public function update() {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            /* $this->form_validation->set_rules('administration_folder_name', 'Newsletter name', 'required');
              ...
              $this->form_validation->set_rules('newsletter_keyword_id', 'Keyword', 'callback_validate_keyword');
              $this->form_validation->set_message('validate_keyword','Please select Max 20 Category!'); */
            $this->form_validation->set_rules('newsletter_sender_name', 'From name', 'trim|required');
            $this->form_validation->set_rules('newsletter_email', 'Newsletter E-mail', 'trim|required|valid_email');
            ...
            if ($this->form_validation->run()) {
                ...
                $data_to_store = array(
                    'newsletter_sender_name' => addslashes($this->input->post('newsletter_sender_name')),
                    'newsletter_email' => addslashes($this->input->post('newsletter_email')),
                    'email' => addslashes($this->input->post('newsletter_email')),
                        //'description' => addslashes($this->input->post('description')),
                );
                //if the insert has returned true then we show the flash message
                if ($this->administration_folder_model->update_administration_folder($id, $data_to_store) == TRUE) {
        ...
        $data['keyword'] = $this->newsletter_keyword_model->get_keyword('', '', 'ASC', '', '', 'Active');
        $data['language'] = $this->newsletter_language_model->get_language('', '', 'ASC', '', '', 'Active');
        $data['administration_folder'] = $this->administration_folder_model->get_administration_folder_by_id($id);
        $data['countries'] = $this->user_model->get_countries('country_name', 'ASC');
        //load the view
        ...
        $data['main_content'] = 'kd2a2a0u1g4/administration_folder/edit';
        $this->load->view('kd2a2a0u1g4/includes/template', $data);
    }
...
//update
    ...
    public function delete() {
        ...
        $get_newsletter_relation = $this->administration_folder_model->get_administration_folder_by_field("newsletter_id", $id);
        //delete emails
        ...
        if (@$get_newsletter_relation[0]['newsletter_relation'] == 'parent') {/*
          ...
          $this->administration_folder_model->delete_administration_folder_by_field('newsletter_rand_id',$get_newsletter_relation[0]['newsletter_rand_id']);
         */
        } else {
            $this->administration_folder_model->delete_administration_folder($id);
        }
        $this->session->set_userdata('flash_message', 'delete');
        redirect('kd2a2a0u1g4/administration-folder/');
    }
...
//edit
    ...
    public function process() {
        ...
        $newsletter_data = $this->administration_folder_model->get_administration_folder_by_field("newsletter_id", $newsletter_id);
        ...
        /* $this->load->helper('email');
          ...
          $host = get_admin_detail();
          $to_email = strtolower($newsletter_data[0]['newsletter_rand_id'])."@".$host['host'];
          $from_email = $newsletter_data[0]['newsletter_email'];
          //echo '<pre>'; print_r($newsletter_data);
          //die;$get_admin_detail['email'], $get_admin_detail['name'])
          $this->email->from($from_email,$newsletter_data[0]['author_name']);

          $this->email->to($to_email);
          $this->email->set_mailtype("html");
          $this->email->subject($newsletter_data[0]['headline']);
          ...
          $this->email->message($newsletter_data[0]['description']);
          if($this->email->send()){
          $this->administration_folder_model->delete_administration_folder($newsletter_id);
          $this->session->set_userdata('flash_message', 'process');
          redirect('kd2a2a0u1g4/administration-folder/');
          }else{
        ...
        //Manual delete and precess
        $check_subscribtion_email = check_subscribtion_email($newsletter_data[0]['newsletter_email'], $newsletter_data[0]['author_name']);

        if ($check_subscribtion_email == true) {

            $pass_blacklist_index = $this->administration_folder_model->get_pass_blacklist_index_by_newsletter_id($newsletter_id);
            if (count($pass_blacklist_index) > 0) {
                $check_blacklist_index = 1;
            } else {
                $check_blacklist_index = increase_blacklist_index($parent_newsletter[0]['headline'], $parent_newsletter[0]['newsletter_rand_id']);
            }
            //echo "check_blacklist_index->". $check_blacklist_index; die;


            if ($check_blacklist_index == 1) {
                ...
                $get_clone = $this->administration_folder_model->get_administration_folder_by_id($newsletter_id);
                //echo count($return_html['count']);
                //die;
                if (count($return_html['count']) > 0) {
                    ...
                    $this->administration_folder_model->update_administration_folder($newsletter_id, $update_data);
                    ...
                    $this->session->set_userdata('flash_message', 'add');
                    redirect('kd2a2a0u1g4/administration-folder/');
                } else if ($check_blacklist_index == -1) {
//                    echo "2";
//                    die;
                    $sn_id = $this->functions->get_newsletter_lsn_id();
                    $data_to_store = array(
                    ...
                    $this->session->set_userdata('flash_message', 'moved_to_admin_inbox');
                    redirect('kd2a2a0u1g4/administration-folder/');
                }
            } else {
                $this->session->set_flashdata('flash_message', 'Mail deleted because of blacklist index!');
            ...
            $this->session->set_flashdata('flash_message', 'Author name and Author E-mail is not in knewdog list!');
            $this->session->set_flashdata('flash_class', 'alert-error');
            redirect('kd2a2a0u1g4/administration-folder/');
        }
    }
...
//edit
    ...
    public function multidelete() {
        ...
        if (!empty($url_segment_5)) {
            redirect('kd2a2a0u1g4/administration_folder/issues/' . $url_segment_5);
        } else {
            redirect('kd2a2a0u1g4/administration_folder');
        }
    }
