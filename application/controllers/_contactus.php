		 ...
		 if ($this->form_validation->run() && $this->recaptcha->getIsValid() )
            {
			   ...
			   $this->load->helper('email');
						  ...
						  if ( ! $this->email->send())
						  {
							...
							$this->session->set_flashdata('flash_class','alert-error');
							  ...
							  if($this->user_model->store_user($data_to_store)){
									...
									$this->session->set_flashdata('flash_message', '<strong>Well done!</strong> Email end.');
									if($redirect == 'home'){
								redirect('home');    	
								}else{
								redirect('contactus');    
				...
				if(!$this->recaptcha->getIsValid()) {
                            ...
                            $this->session->set_flashdata('flash_message','incorrect captcha!');
							$this->session->set_flashdata('flash_class', 'alert-error');
                        } else {
                            $this->session->set_flashdata('flash_message','incorrect credentials!');
							$this->session->set_flashdata('flash_class', 'alert-error');
                        }
