...
class Newsletter extends CI_Controller {
    ...
    public function index() {
        ...
        if (!empty($mynl_order)) {
            ...
            $mynl_order_type = 'ASC';
            $mynl_order = $mynl_order;
        } else {
            $mynl_order = 'newsletter_name';
            $mynl_order_type = 'ASC';
        }
        ...
        if ($this->uri->segment(3) || $this->input->post('form') == 'section_2') {
            $data['mynl_tab'] = 'tab_2';
        } else {
            $data['mynl_tab'] = 'tab_1';
        }
        ...
        if ($search_string !== false && $order !== false || $this->uri->segment(2) == true) {
            ...
            $data['selected_newsletter_rating_id'] = $rating_id;
            ...
            $data['count_products'] = $this->newsletter_model->count_newsletter_front($search_string, $order, $language_id, $newsletter_category, $rating_id, $author_country, $author_zipcode, $where_field, $where_value);
            ...
            $data['search_string_selected'] = null;
            $data['order'] = 'id';
            ...
            $data['order'] = null;
            $data['selected_language_id'] = null;
            $data['selected_newsletter_rating_id'] = null;
            $data['selected_newsletter_category_id'] = null;
            $data['selected_author_country'] = null;
            $data['selected_author_zipcode'] = null;

            //fetch sql data into arrays
            $data['count_newsletter'] = $this->newsletter_model->count_newsletter_front('', '', '', '', '', '', '', $where_field, $where_value);
            @$data['newsletter'] = $this->newsletter_model->get_newsletter_front('', $order, $order_type, $config['per_page'], $limit_end, 'Active', $where_field, $where_value, $language_id, $newsletter_category_id, $rating_id, $author_country, $author_zipcode);
        ...
        if ($mynl_search_string !== false && $mynl_order !== false || $this->uri->segment(2) == true) {
            ...
            $data['mynl_selected_newsletter_rating_id'] = $mynl_rating_id;
            ...
            $mynl_config['total_rows'] = $data['mynl_count_products'];
            ;
            //fetch sql data into arrays
            if ($mynl_search_string) {
                if ($mynl_order) {
                    ...
                    $data['mynl_newsletter'] = $this->newsletter_model->get_newsletter_front($mynl_search_string, '', $mynl_order_type, $mynl_config['per_page'], $mynl_limit_end, 'Active', $mynl_where_field, $mynl_where_value, $mynl_language_id, $mynl_newsletter_category, $mynl_rating_id, $mynl_author_country, $mynl_author_zipcode, $mynl_where_in_field, $mynl_where_in_value);
                }
            } else {
                if ($mynl_order) {
                    $data['mynl_newsletter'] = $this->newsletter_model->get_newsletter_front('', $mynl_order, $mynl_order_type, $mynl_config['per_page'], $mynl_limit_end, 'Active', $mynl_where_field, $mynl_where_value, $mynl_language_id, $mynl_newsletter_category, $mynl_rating_id, $mynl_author_country, $mynl_author_zipcode, $mynl_where_in_field, $mynl_where_in_value);
                } else {
                    $data['mynl_newsletter'] = $this->newsletter_model->get_newsletter_front('', '', $mynl_order_type, $mynl_config['per_page'], $mynl_limit_end, 'Active', $mynl_where_field, $mynl_where_value, $mynl_language_id, $mynl_newsletter_category, $mynl_rating_id, $mynl_author_country, $mynl_author_zipcode, $mynl_where_in_field, $mynl_where_in_value);
                }
            }
        } else {
            ...
            $data['mynl_selected_newsletter_rating_id'] = null;
            $data['mynl_selected_author_country'] = null;
...
//index
    ...
    function specific() {
        ...
        if (!$this->session->userdata('is_logged_in')) {
            $this->session->set_flashdata('flash_message', '' . _clang(OUR_EXTENSIVE_NEWSLETTER) . ' <a href="' . site_url('signup') . '">' . _clang(SIGN_UP_NOW_MESSAGE) . '</a>, ' . _clang(ITS_FREE_MESSAGE) . '');
        ...
        $data["last_newsletter_issue"] = $this->newsletter_model->just_get_last_sn_id($data['newsletter'][0]["newsletter_rand_id"]);
        $where_field = array("newsletter_relation", "newsletter_rand_id");
        $where_value = array("child", $data['newsletter'][0]['newsletter_rand_id']);
        $data['archive_newsletter'] = $this->newsletter_model->get_newsletter_front_to('', 'newsletter_id', 'DESC', '', '', 'Active', $where_field, $where_value, '', '', '', '', '');
//        echo "<pre>";
//        print_r($data['archive_newsletter']);
//        exit;
        $data['newsletter_review'] = $this->newsletter_model->get_newsletter_review('', '', 'DESC', '2', '0', '', array(), array(), $id);
    ...
    function display_rate() {
        ...
        /* $this->load->model('subscribe_model');
          ...
          $where_s_value = array($user_id,$id);
          $subscribed = $this->subscribe_model->get_subscribe('', '', '', '', '',$where_s_field,$where_s_value);
          if(count($subscribed) == 0){
          $rediret_url = $this->session->flashdata('redirect_url');
          $this->session->set_flashdata('flash_message', 'Opps! you are not subsribed to add reviews! please Subscribe First.');
          $this->session->set_flashdata('flash_class', 'alert-error');
        ...
        //echo "<pre>";print_r($data['count_products']);exit;
        ...
        $get_rate = $this->newsletter_model->get_rate_by_user($id);
        $data['newsletter'] = $this->newsletter_model->get_newsletter_by_id_front($id);
        $data['newsletter_review'] = $this->newsletter_model->get_newsletter_review('', '', 'DESC', $config['per_page'], $limit_end, '', $wherefield, $wherevalue, $id);
        $data['get_rate'] = $get_rate;
    ...
    function addreview() {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($this->newsletter_model->store_newsletter_review($data_to_store)) {
                $avg_rate = get_average_rate($this->input->post('join_newsletter_id'));
                $rate = array("ratings" => $avg_rate["avg_round"]);
                //add average rate
                $this->newsletter_model->update_newsletter($this->input->post('join_newsletter_id'), $rate);

                $this->session->set_flashdata('flash_class', 'alert-success');
                $this->session->set_flashdata('flash_message', '<strong>' . _clang(WELL_DONE) . '</strong> ' . _clang(YOUR_REVIEW_ADDED_WITH) . '');
                //echo "if";die;
    ...
    function editreview() {
        $newsletter_review_id = $this->input->post('newsletter_review_id');
        $title = $this->input->post('title');
        $message = $this->input->post('message');
        $id = $this->input->post('join_newsletter_id');
        $this->form_validation->set_rules('rate', 'Rate', 'required');
        ...
        if ($this->form_validation->run()) {
            ...
            if ($this->newsletter_model->update_newsletter_review($newsletter_review_id, $data_to_store)) {
                $avg_rate = get_average_rate($this->input->post('join_newsletter_id'));
                $rate = array("ratings" => $avg_rate["avg_round"]);
                //add average rate
                $this->newsletter_model->update_newsletter($this->input->post('join_newsletter_id'), $rate);
                ...
                $this->session->set_flashdata('flash_class', 'alert-success');
                $this->session->set_flashdata('flash_message', '<strong>' . _clang(WELL_DONE) . '</strong> ' . _clang(YOUR_REVIEW_UPDATED_WITH) . '');
                //echo "if";die;
                redirect("newsletter/display-rate/" . $id);
            } else {
                $this->session->set_flashdata('flash_class', 'alert-error');
