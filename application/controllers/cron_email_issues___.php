...
//
    ...
    public function process() {
        ...
        for ($r = 0; $r < count($rarray); $r++) {
            ...
            $raw_email = base64url_encode($rarray[$r]['toaddress']); //$this->uri->segment(5);
...
//bhushan changes 
        ...
        for ($c = 0; $c < count($reminder_mail_data_end_of_term); $c++) {
            ...
            for ($m = 0; $m < count($user_details); $m++) {
                $userArray['username'] = $user_details[$m]['username'];
                $userArray['account_type'] = $user_details[$m]['type_of_membership'];
            }
            echo "<pre>";
            print_r($userArray['username']);
            exit;
            ...
            if ($reminder_mail_data_end_of_term[$c]['before_four_week'] == "NO") {
