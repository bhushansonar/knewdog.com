...
class Myknewdog extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->uri->segment(2) || $this->input->post('form') == 'section_1') {
            $data['mynl_tab'] = 'tab_1';
        } else if ($this->uri->segment(2) || $this->input->post('form') == 'section_2') {
            $data['mynl_tab'] = 'tab_2';
        ...
        $data['language_keyword_to_email'] = $this->language_keyword_model->get_field_data('language_keyword', 'language_define', 'en', 'EMAIL_TO');
    ...
    function schedule() {
        ...
        if ($this->input->server('REQUEST_METHOD') ===
            ...
            if ($this->input->post('sending') ==
                ...
                $this->form_validation->set_rules('monthly_on', 'Day of the month or Weekday', 'required');
            }
            ...
            if ($this->session->userdata("type_of_membership") == "PRE2") {
                $this->form_validation->set_rules('sending', 'schedule', 'trim|required|count_schedule[' . $user_id . '.999]');
            }
            $this->form_validation->set_error_delimiters('<div class="alert alert-error"><a class="close" data-dismiss="alert">&#215;</a><strong>', '</strong></div>');
            ...
            if ($this->form_validation->run()) {
                ...
                foreach ($array as $k => $v) {
                    if (in_array($k, array('form', 'time_zone', 'to_email'))) {
    ...
    function schedule_edit() {
        ...
        if ($this->input->server('REQUEST_METHOD') ===
            ...
            if ($this->form_validation->run()) {
                ...
                $monthly_on = '';
                $day_of_the_month = '';
                $monthly_weekday_count = '';
                $monthly_weekday_day = '';
                if ($this->input->post('sending') == 'Monthly') {
                    $monthly_on = $this->input->post('monthly_on');
                    if ($monthly_on == 'day_of_the_month') {
                        $day_of_the_month = $this->input->post('day_of_the_month');
                    }
                    if ($monthly_on == 'weekday') {
                        $monthly_weekday_count = $this->input->post('monthly_weekday_count');
                        $monthly_weekday_day = $this->input->post('monthly_weekday_day');
                    }
                }
                $ends =
                ...
                $array =
                        array(
                            ...
                            'monthly_weekday_count' => $monthly_weekday_count,
                            'monthly_weekday_day' => $monthly_weekday_day,
        ...
        if ($type_of_membership == 'FREE') {
            ...
            if (valid_email($user_email)) {
                ...
                if (isset($email_template_content_free_account[0]['description_' . $session_lang]) &&
                    ...
                    $this->email->subject($email_template_content_free_account[0]['subject_' . $session_lang]);
                    ...
                    $this->email->subject($email_template_content_free_account[0]['subject_en']);
                    $cancellation_email_content_for_free =
...
//						  	$this->email->message($message);
            ...
            if (valid_email($admin_email)) {
                ...
                if (isset($email_template_content_cancle_admin[0]['description_' . $session_lang]) &&
                    ...
                    $this->email->subject($email_template_content_cancle_admin[0]['subject_' . $session_lang]);
                    ...
                    $this->email->subject($email_template_content_cancle_admin[0]['subject_en']);
...
// redirect('home');
        ...
        if ($type_of_membership != 'FREE') {
            ...
            if (valid_email($email)) {
                ...
                $this->email->to($email);
                $this->email->set_mailtype("html");
//                $this->email->subject('Your knewdog account has been downgraded!');
                ...
                $replace = array('{account_type}', '{cancle_username}', '{cancle_amount}', '{user_email}');
                $with = array("{$mail_data['account_type']}", "{$mail_data['cancle_username']}", "{$mail_data['cancle_amount']}", "{$mail_data['user_email']}");
...
//						  	$this->email->message($message);
            ...
            if (valid_email($email)) {
                ...
                $this->email->to($email);
                $this->email->set_mailtype("html");
//                $this->email->subject("User has cancelled it's account on knewdog!");
