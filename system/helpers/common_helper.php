...
//print_r($parsed);
        ...
        return array("hostname" => $hostname, "path" => $path, "query" => $query);
    }

}

if (!function_exists('get_youtube_code_only')) {

    function get_youtube_code_only($videolink) {
        $ytarray = explode("/", $videolink);
        $ytendstring = end($ytarray);
        $ytendarray = explode("?v=", $ytendstring);
        $ytendstring = end($ytendarray);
    ...
    function increase_blacklist_index($subject, $newsletter_rand_id) {
        ...
        if ($newsletter_parent_data[0]['blacklist_flag'] == 'NO') {
            ...
            if (stristr($subject, 'SPAM') !== FALSE) {
                ...
                $email = array('blacklist@knewdog.com');
                //$email = 's.bhushan011@gmail.com';
                //$email = 'vijay@amutechnologies.com';
                /*
                  bhushan changes
                 */
...
//                }
                ...
                if (!$send) {
                    echo "if->" . $print_error = $ci->email->print_debugger();
                    log_message('error', $print_error, TRUE);
                } else {
                    echo "else->" . $print_error = $ci->email->print_debugger();
                    log_message('info', $print_error, TRUE);
                }
                $ci->email->clear(TRUE);
                return -1;
            } else {
...
if (!function_exists('increase_blacklist_index_for_administration_folder')) {
    ...
    function get_random_adds($user_id) {
        ...
        if ($user_data[0]['no_ads'] == 'NO') {
            if (!empty($user_data[0]['user_interests'])) {
                ...
                $add_result = $ci->advertisement_model->get_advertisement_active();
                $rand_key = array_rand($add_result, 1);
                $rand_add = $add_result[$rand_key];
            }
...
/*
 * *
 * Use to get schedule_newsletter_to_choose_limit

 * return int
 * */
if (!function_exists('schedule_newsletter_to_choose_limit')) {

    function schedule_newsletter_to_choose_limit($type_of_membership) {
        switch ($type_of_membership) {
            case 'FREE':
                $allow_subscription = 5;
                break;
            case 'PRE1':
                $allow_subscription = 99;
                break;
            case 'PRE2':
                $allow_subscription = 999;
            default:
                $allow_subscription = 5;
                break;
        }
        return (int) $allow_subscription;
    }
