...
//For logged in user, get details from google using access token
            ...
            if (!empty($user)) {
               ...
               if($user['email']==$get_users[0]['primary_email']){
                   echo "hi";exit;
               }exit;
            ...
            for ($m = 0; $m < count($replace_href); $m++) {
                ...
                for ($i = 0; $i < count($return_string); $i++) {
                    ...
                    if ($posi == true) {
                        ...
                        if ($count > 0) {
                            $pre_coun[] = array($replace_href[$m], $count);
                        }
                    }
                }
            }
...
if (!function_exists('send_mail_attachment')) {
    ...
    function send_mail_attachment($email, $type_of_member, $yesterday = false) {
        ...
        if (valid_email($email)) {
            ...
            $ci->email->from("noreply@knewdog.com", $get_admin_detail['name']);
            $ci->email->to($email);
            $ci->email->set_mailtype("html");
            ...
            $link = '<a href="' . site_url() . 'signin/mynewsletter/' . base64url_encode($user_data[0]['primary_email']) . '">www.knewdog.com/mynewsletter</a>';
...
if (!function_exists('summary_lang')) {
    ...
    function summary_lang($word) {
       // echo $word;exit;
        ...
        //echo "<pre>";print_r($lang_data);exit;
        $session_lang = $ci->session->userdata('language_shortcode');
